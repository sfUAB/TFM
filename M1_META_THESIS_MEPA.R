...
files_INMA <- list.files(path = "./data/INMA/results/phenols/model1", 
                         pattern = "PACE_phthalates_INMA_EWAS_Model(A|B|C)_(mepa)_mraw_(creat_total_log2)_.*", 
                         full.names = TRUE)
files_INMA #3

files_NELA <- list.files(path = "./data/NELA/results/phenols/model1", 
                         pattern = "PACE_phthalates_NELA_EWAS_Model(A|B|C)_(ad.*)_(MPB)(_media)?_S24_.*", 
                         full.names = TRUE)
files_NELA #3 files

files <- c(files_INMA, files_NELA)
files #6 total files 

# Define a new directory for the modified files
output_dir <- "./data/renamed_files_forSFTHESIS_MEPA"
...
metafiles <- list(
  'Meta1A' = c('Model1A_INMA_MEPA', 'Model1A_NELA_MEPA' ),
  
  'Meta1B' = c('Model1B_INMA_MEPA','Model1B_NELA_MEPA'),
  
  'Meta1C' = c( 'Model1C_INMA_MEPA','Model1C_NELA_MEPA'))
...
results_gwama <- 'metaanalysis/GWAMA_Results_Phenols_Sfraley_thesis/Model_1_MEPA'
