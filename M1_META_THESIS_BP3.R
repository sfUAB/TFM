...
files_INMA <- list.files(path = "./data/INMA/results/phenols/model1", 
                         pattern = "PACE_phthalates_INMA_EWAS_Model(A|B|C)_(bp3)_mraw_(creat_total_log2)_.*", 
                         full.names = TRUE)
files_INMA #3

files_NELA <- list.files(path = "./data/NELA/results/phenols/model1", 
                         pattern = "PACE_phthalates_NELA_EWAS_Model(A|B|C)_(ad.*)_(BP3)(_media)?_S24_.*", 
                         full.names = TRUE)
files_NELA #3
...
output_dir <- "./data/renamed_files_forSFTHESIS_Bp3"
...
metafiles <- list(
  ...
  'Meta1B' = c('Model1B_INMA_BP3', 
               'Model1B_NELA_BP3'),
  
  'Meta1C' = c( 'Model1C_INMA_BP3', 
                'Model1C_NELA_BP3'))
...
results_gwama <- 'metaanalysis/GWAMA_Results_Phenols_Sfraley_thesis/Model_1_BP3'
