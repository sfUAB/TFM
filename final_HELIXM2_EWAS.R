# =========================================================================== #
#sylviefraley
#Model2, prenatal exposure vs. child methylation 
# =========================================================================== #
...
pheno <- read.table('../db/pheno_maternal/pheno_ALL.txt', sep = '\t', header = T, stringsAsFactors = T)
dim(pheno)#1114
...
summary(raw_values$hs_creatinine_mg)
no_NA_creatinine <- raw_values[!is.na(raw_values$hs_creatinine_mg), ] #224 NAs for creatinine removed
dim(no_NA_creatinine) #1106 remaining
...
#looking for descriptives of compounds of interest in mothers
id_cols <- c("cohort", "helix_id", "sample_id", "subject_id", "hs_creatinine_mg", "hs_creatinine_mdesc")
matching_cols <- names(no_NA_creatinine)[
  grepl("bpa_m(desc)?$|prpa_m(desc)?$|trcs_m(desc)?$", names(no_NA_creatinine), ignore.case = TRUE)
]
keep_cols <- c(id_cols, matching_cols)
subset_desc <- no_NA_creatinine[, keep_cols]

# 2. Filter rows where any "desc" column has value 2
desc_cols <- grep("desc", names(subset_desc), ignore.case = TRUE, value = TRUE)
print(desc_cols)

descriptives <- data.frame(
  chemical = c("bpa", "prpa", "trcs"),
  N_below_LOD = 0,
...
for (i in 1:nrow(subset_desc)){
  for(chem in names(lods)){
    level_col <- paste0("hs_", chem, "_m")
    desc_col  <- paste0("hs_", chem, "_mdesc")
...
print(count_imputed) #57 imputed
print(descriptives)
...
for (i in 1:nrow(subset_desc)){
  for (chem in names(lods)) {
    ...
    if (level_col %in% names(subset_desc) && !is.na(subset_desc[i,level_col]) && (subset_desc[i, desc_col]) %in% c(1, 2)) {
      normed <- as.numeric(subset_desc[i, level_col]) / as.numeric(subset_desc[i, "hs_creatinine_mg"])
      if (subset_desc[i, desc_col] == 1){
        if (chem == "bpa"){
          BPA_normed <- c(BPA_normed, normed)
        }
        if (chem == "prpa"){
          PRPA_normed <- c(PRPA_normed, normed)
        }
        if (chem == "trcs"){
          trcs_normed <- c(trcs_normed, normed)
        }
      }
      if (!is.na(normed) && normed > 0) {
        ...
        subset_desc[i, level_col] <- NA  
      }
    }
  }
}
summary(subset_desc$hs_bpa_m) #now negative values bc log2 transformed

#create descriptives with values not log2 transformed
descriptives["bpa", "min"] <- min(BPA_normed, na.rm = TRUE)
descriptives["prpa", "min"] <- min(PRPA_normed, na.rm = TRUE)
descriptives["trcs", "min"] <- min(trcs_normed, na.rm = TRUE)

descriptives["bpa", "p5"] <- quantile(BPA_normed, 0.05, na.rm = TRUE)
descriptives["prpa", "p5"] <- quantile(PRPA_normed, 0.05, na.rm = TRUE)
descriptives["trcs", "p5"] <- quantile(trcs_normed, 0.05, na.rm = TRUE)

descriptives["bpa", "p25"] <- quantile(BPA_normed, 0.25, na.rm = TRUE)
descriptives["prpa", "p25"] <- quantile(PRPA_normed, 0.25, na.rm = TRUE)
descriptives["trcs", "p25"] <- quantile(trcs_normed, 0.25, na.rm = TRUE)

descriptives["bpa", "p50"] <- quantile(BPA_normed, 0.5, na.rm = TRUE)
descriptives["prpa", "p50"] <- quantile(PRPA_normed, 0.5, na.rm = TRUE)
descriptives["trcs", "p50"] <- quantile(trcs_normed, 0.5, na.rm = TRUE)

descriptives["bpa", "p75"] <- quantile(BPA_normed, 0.75, na.rm = TRUE)
descriptives["prpa", "p75"] <- quantile(PRPA_normed, 0.75, na.rm = TRUE)
descriptives["trcs", "p75"] <- quantile(trcs_normed, 0.75, na.rm = TRUE)

descriptives["bpa", "p95"] <- quantile(BPA_normed, 0.95, na.rm = TRUE)
descriptives["prpa", "p95"] <- quantile(PRPA_normed, 0.95, na.rm = TRUE)
descriptives["trcs", "p95"] <- quantile(trcs_normed, 0.95, na.rm = TRUE)

descriptives["bpa", "max"] <- max(BPA_normed, na.rm = TRUE)
descriptives["prpa", "max"] <- max(PRPA_normed, na.rm = TRUE)
descriptives["trcs", "max"] <- max(trcs_normed, na.rm = TRUE)

descriptives["bpa", "mean"] <- mean(BPA_normed, na.rm = TRUE)
descriptives["prpa", "mean"] <- mean(PRPA_normed, na.rm = TRUE)
descriptives["trcs", "mean"] <- mean(trcs_normed, na.rm = TRUE)

descriptives["bpa", "SD"] <- sd(BPA_normed, na.rm = TRUE)
descriptives["prpa", "SD"] <- sd(PRPA_normed, na.rm = TRUE)
descriptives["trcs", "SD"] <- sd(trcs_normed, na.rm = TRUE)

descriptives["bpa", "IQR"] <- IQR(BPA_normed, na.rm = TRUE)
descriptives["prpa", "IQR"] <- IQR(PRPA_normed, na.rm = TRUE)
descriptives["trcs", "IQR"] <- IQR(trcs_normed, na.rm = TRUE)

print(descriptives)
write_xlsx(descriptives, file.path(getwd(), "descriptives_M2.xlsx"))


#now correlation 
#want to compare only levels of chemical in case where desc = 1. At this point, subset_desc has already done imputation and creatinine transformation. 
bpa_values <- c()
prpa_values <- c()
trcs_values <- c()
for (i in 1:nrow(subset_desc)){
  if (subset_desc[i, "hs_bpa_mdesc"] %in%  c(1,2) && subset_desc[i, "hs_prpa_mdesc"] %in%  c(1,2) && subset_desc[i, "hs_trcs_mdesc"] %in%  c(1,2)) {
    bpa_values <- c(bpa_values, subset_desc[i, "hs_bpa_m"])
    prpa_values <- c(prpa_values,subset_desc[i, "hs_prpa_m"])
    trcs_values <- c(trcs_values,subset_desc[i, "hs_trcs_m"])
  }
  
}
correlation <- matrix(nrow = 3, ncol = 3)
rownames(correlation) <- c("BPA", "PRPA", "TRCS")
colnames(correlation) <- c("BPA", "PRPA", "TRCS")
print(correlation)
correlation[1,2] <- cor(as.numeric(bpa_values), as.numeric(prpa_values), use = "complete.obs")
correlation[2, 1] <- cor(as.numeric(prpa_values), as.numeric(bpa_values), use = "complete.obs")

correlation[2, 3] <- cor(as.numeric(prpa_values), as.numeric(trcs_values), use = "complete.obs")
correlation[3, 2] <- cor(as.numeric(trcs_values), as.numeric(prpa_values), use = "complete.obs")

correlation[3, 1] <- cor(as.numeric(trcs_values), as.numeric(bpa_values), use = "complete.obs")
correlation[1, 3] <- cor(as.numeric(bpa_values), as.numeric(trcs_values), use = "complete.obs")


correlation[1, 1] <- cor(as.numeric(bpa_values), as.numeric(bpa_values), use = "complete.obs")
correlation[2, 2] <- cor(as.numeric(prpa_values), as.numeric(prpa_values), use = "complete.obs")
correlation[3, 3] <- cor(as.numeric(prpa_values), as.numeric(prpa_values), use = "complete.obs")

print(correlation)
getwd()
correlation <- data.frame(correlation)
write_xlsx(correlation, file.path(getwd(), "correlations_M2.xlsx"))

##-----------------------------------------------------------------
#compare mine to pheno 
  
names(pheno)[names(pheno) == "HelixID"] <- "helix_id"
names(pheno)[names(pheno) == "TRICLOSAN_mraw_creat_preg_log2"] <- "trcs_mraw_creat_preg_log2"

#chemical levels 
pheno_col <- c(
  "BPA_mraw_creat_preg_log2", "PARABENS3_PRPA_mraw_creat_preg_log2", "trcs_mraw_creat_preg_log2")


common_ids <- intersect(subset_desc$helix_id, pheno$helix_id)
length(common_ids) #915 IDs common to both dataframes
...
write_xlsx(comparison, file.path(getwd(), "imputation_comparison_M2.xlsx"))
...
names(pheno)[names(pheno) == "BPA_mraw_creat_preg_log2"] <- "hs_bpa_m"
names(pheno)[names(pheno) == "PARABENS3_PRPA_mraw_creat_preg_log2"] <- "hs_prpa_m"
names(pheno)[names(pheno) == "trcs_mraw_creat_preg_log2"] <- "hs_trcs_m"
update_cols <- c("hs_bpa_m", "hs_prpa_m", "hs_trcs_m")
id_col_1 <- "helix_id"
...
# Replace this example of metabolites

traits <- c("hs_bpa_m", "hs_prpa_m", "hs_trcs_m")
length(traits)
...
cont_vars <- c('edadm', 'prepregBMI', 'age', 'gestage', 'NK', 'Bcell', 'CD4T', 'Gran', 'CD8T', 'Mono',
...
dir.create('M2_persistenteffects_newimputation')
...
# Round decimals
pheno$prepregBMI <- round(pheno$zBMI, digits = 3)
...
pheno <- pheno[, phenocols]
dim(pheno) #28 columns, same number of rows (1114)

# check missings in phenols
summary(pheno$hs_prpa_m, useNA="ifany")#14 NA
summary(pheno$hs_bpa_m, useNA="ifany")#11 NA
summary(pheno$hs_trcs_m, useNA="ifany")#none?


# Select complete cases to do the analysis, check this matches descriptives, yes it does 
# If you have >5% missing for any of the variables, check with us before proceeding
pheno <- pheno[complete.cases(pheno), ] #remove rows with any NA values in any column, should be 34 less
dim(pheno)#840 now instead of 1114
colnames(pheno)
colnames(subset_desc)
...
dim(beta_matrix) #now 840 columns to match the samples in pheno1
...
for (trait in traits) {
  ...
  ModelA <- as.formula(paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'))
  ModelB <- as.formula(paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + Mono + Bcell + CD4T + CD8T +  Gran + NK + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'))
  ModelC <- as.formula(paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + Mono + Bcell + CD4T + CD8T +  Gran + NK + gestage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'))
  ...
  formulas <- c(paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'),
                paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + Mono + Bcell + CD4T + CD8T +  Gran + NK + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'),
                paste0('CpGi ~ ', trait, ' + edadm + prepregBMI + parity + estudios3c + smok_preg + sexo + age + Mono + Bcell + CD4T + CD8T +  Gran + NK + gestage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10'))
  
  sapply(seq_along(models), function(i) {
    
    # results <- mclapply(beta_lists, apply_model, phen = pheno, form = as.formula(formulas[i]), mc.cores = mccores)
